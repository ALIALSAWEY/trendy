...
const Category = ({addProduct, isAuthenticated, addProductSingle, clearCart, showMessage, hideMessage, setLoader, disableLoader }) => {
    ...
    const getProducts = () => {
        ...
        if(window.location.href.includes('search')) {
            axios.get(`http://localhost:5000/product/search/${search}?limit=9&skip=${skip}`)
            .then( ({ data }) => {
            ...
            axios.get(`http://localhost:5000/product/all?limit=9&skip=${skip}`)
            .then( ({ data }) => {
                if(data.length === 0) {
                    setHasMore(false)
                    disableLoader()
            ...
            axios.get(`http://localhost:5000/product/category/${category}?limit=9&skip=${skip}`)
            .then( ({ data }) => {
